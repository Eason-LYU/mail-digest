...
/** 2026-11-01 → 11/01：只用于「改期」那一行，让改前/改后一眼可比 */
function fmtDay(v) {
...
export function buildDigest(messages, meta = {}) {
  ...
  const opText = (o) => {
    switch (o?.op) {
      ...
      case "set_due": return `🔁 改期：${o.title}${o.from ? `　${fmtDay(o.from)} → ` : " → "}${fmtDay(o.to ?? o.due) || "(未指定)"}`;
  ...
  // 收件箱看起来还没同步完时，**必须在日报里写明**——否则"0 封新邮件"会被当成"今天真的没邮件"。
  // 2026-09-23 13:25 真实发生：Outlook 缓存停在 16 小时前，日报却报"0 封新邮件"。
  const staleNotice = meta.staleNotice ? String(meta.staleNotice) : "";
  // 只是"收信后确实没有更新的邮件"这类**事实**：不报警，但写清楚覆盖到哪里，
  // 免得"0 封新邮件"被当成"今天真的没邮件"（2026-09-23 13:25 就是这么被误读的）。
  const staleInfo = !staleNotice && meta.staleInfo ? String(meta.staleInfo) : "";
  const staleHtml = staleNotice ? `
  <div style="background:#fffaeb;border-left:4px solid #f79009;padding:10px 12px;margin:0 0 16px 0;font:13px/1.6 'Microsoft YaHei',sans-serif;color:#93370d;">
    ⚠️ <b>邮件可能没读全</b>：${esc(staleNotice)}
  </div>` : (staleInfo ? `
  <div style="background:#f9fafb;border-left:4px solid #98a2b3;padding:8px 12px;margin:0 0 16px 0;font:12px/1.6 'Microsoft YaHei',sans-serif;color:#475467;">
    ℹ️ ${esc(staleInfo)}
  </div>` : "");
  const staleMd = staleNotice
    ? [`> ⚠️ **邮件可能没读全**：${staleNotice}`, ""]
    : (staleInfo ? [`> ℹ️ ${staleInfo}`, ""] : []);
  // 改期是可逆的：把"改回旧日期"的原话写在下面，用户照抄一句就能撤销。
  // （2026-09-23 事故的另一半：日期被改掉之后，日报只写"→ 新日期"，
  //   旧日期再也找不回来，用户不知道该回什么才能恢复。）
  const undoLines = cmdApplied
    .filter((o) => (o.op === "set_due" ? o.from : o.becauseAddIntent))
    .map((o) => (o.becauseAddIntent ? `🗑 撤销「${o.title}」` : `↩️ ${o.title} 改回 ${fmtDay(o.from)}`));
  ...
  <div style="font:13px/1.9 'Microsoft YaHei',sans-serif;color:#344054;">
    ...
    ${undoLines.length ? `<br><span style="color:#475467;">改错了？回信写「${esc(undoLines.join("；"))}」即可撤销</span>` : ""}
  </div>
  ...
  const cmdMd = (cmdApplied.length || cmdFailed.length) ? [
    ...
    ...(undoLines.length ? [`- ↩️ 改错了？回信写「${undoLines.join("；")}」即可撤销`] : []),
    "",
  ] : [];
