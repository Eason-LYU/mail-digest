...
// 备份文件名用的时间戳。
// 只用毫秒精度是不够的：同一毫秒内写两次会**同名**，后一份把前一份覆盖掉，
// 结果就是"备份比预期少"。在快的机器上（CI 的 runner）这几乎必然发生，已真实踩到。
//
// 两个都必须做到，否则"文件名排序 = 时间排序"这个前提就破了（rotate/listBackups 都靠它）：
//   1) 同一毫秒内加自增序号；
//   2) 序号**零填充**，且**每一份都带**（包括第一份）——因为字典序里 '-' 排在 '.' 前面，
//      不带序号的那份会在 ".json" 处"赢过"带序号的，把最新的一份挤到后面去。
// 2026-09-21 CI 上连着踩了这两个坑，`test/statefile.test.mjs` 里有用例钉死。
let lastMs = 0;
let seq = 0;
function stamp() {
  const ms = Date.now();
  if (ms === lastMs) seq += 1; else { lastMs = ms; seq = 0; }
  const iso = new Date(ms).toISOString().replace(/[:.]/g, "-");
  return `${iso}-${String(seq).padStart(6, "0")}`;
}
