...
export function nextWindowStart({ since, newestSeenMailAt }) {
  if (newestSeenMailAt) {
    ...
    if (!Number.isNaN(d.getTime())) return new Date(d.getTime() + 1000).toISOString();
  }
 ...
 */
export function shouldRetryEmptyRead({ count = 0, newestSeenMailAt = null, since = null } = {}) {
  if (count > 0) return false;                    // 读到了就不用重试
  if (!since) return true;                        // 没有窗口起点（很少见）也值得重读一次
  const t = Date.parse(newestSeenMailAt || "");
  if (Number.isNaN(t)) return true;               // 连"见到的最新邮件"都没有 → 更像没同步完
  return t < new Date(since).getTime();           // 收件箱里最新的邮件还早于窗口起点 → 可疑
}