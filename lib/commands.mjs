...
import { removeHandled } from "./pending.mjs";
...
export function applyOps(ledger, ops, now = new Date(), { pending = [] } = {}) {
  const applied = [];
  const failed = [];
...
export function buildCommandPrompt(mail, ledger, pending = []) {
  const open = ledger.items.filter((i) => i.status === "open");
 ...
 */
export async function processCommandMails(mails, { ledger, pending = [] }, {
  ...
  for (const m of mails) {
    ...
    if (isAckReply(written)) {
      ...
      const res = applyOps(ledger, [{ op: "handled", all: true }], now, { pending: pendingList });
    ...
    const raw = await tr.ask(buildCommandPrompt(m, ledger, pendingList));
    ...
    const res = applyOps(ledger, ops, now, { pending: pendingList });
    pendingList = res.pending ?? pendingList;
    applied.push(...res.applied);
    failed.push(...res.failed);
  }
