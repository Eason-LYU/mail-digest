...
import { removeHandled, addPending } from "./pending.mjs";
import { openItems } from "./todos.mjs";
...
export function applyOps(ledger, ops, now = new Date(), { pending = [], watch = [] } = {}) {
  const applied = [];
  const failed = [];
  let pendingList = pending;

  const findByRef = (ref, seq) => {
    // 用户说"待办3"时给的是序号：按日报里的顺序解析（确定性，不靠模型猜）
    if (Number.isInteger(seq) && seq >= 1) {
      const picked = openItems(ledger, now, { limit: 200 }).shown[seq - 1];
      if (picked) return ledger.items.find((i) => i.key === picked.key) || null;
    }
    if (!ref) return null;
    ...
    // 前缀匹配要求够长且唯一，避免 "m:" 这种短前缀一次命中一堆
    const byPrefix = open.filter((i) => i.key.startsWith(r));   // 但必须唯一（见下面）
    return open.find((i) => i.key === r)
      || (byPrefix.length === 1 ? byPrefix[0] : null)
  ...
  return { applied, failed, pending: pendingList };
}
...
export function buildCommandPrompt(mail, ledger, pending = [], watch = []) {
  // ⚠️ 必须带上**用户看到的那个序号**：用户会说"待办3，5，9删除"，
  // 而日报里的编号就是 openItems 的顺序（按截止日升序、无期限的排后面）。
  // 以前不传序号，模型只能自己猜位置 —— 2026-09-22 真实删错了一条待办。
  const open = openItems(ledger, new Date(), { limit: 200 }).shown;
  return JSON.stringify({
    邮件主题: String(mail?.subject || "").slice(0, 200),
    邮件正文: stripQuoted(mail?.bodyPreview).replace(/\s+/g, " ").slice(0, 1500),
    当前未完成待办: open.map((i, idx) => ({ 序号: idx + 1, id: i.key, title: i.title, due: i.due || null })),
 ...
 */
export async function processCommandMails(mails, { ledger, pending = [], watch = [] }, {
  ...
  for (const m of mails) {
    ...
    if (isAckReply(written)) {
      ...
      const res = applyOps(ledger, [{ op: "handled", all: true }], now, { pending: pendingList, watch });
    ...
    const raw = await tr.ask(buildCommandPrompt(m, ledger, pendingList, watch));
    const ops = parseJsonArrayLoose(raw);
    if (!ops) {
    ...
    const res = applyOps(ledger, ops, now, { pending: pendingList, watch });
    pendingList = res.pending ?? pendingList;
    applied.push(...res.applied);
    failed.push(...res.failed);
  ...
  return { applied, failed, mails: mails.length, pending: pendingList, stats: st };
}
